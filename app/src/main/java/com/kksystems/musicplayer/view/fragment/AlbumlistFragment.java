...
//        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(requireContext());
    ...
    private void setEventListener() {
        listView.addOnItemTouchListener(
                new RecyclerViewItemClickListener(requireContext(), new RecyclerViewItemClickListener.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, int position) {
                        String targetMediaId = mModel.getMediaList().getValue().get(position).getMediaId();

                        final NavigationFragment parent = (NavigationFragment) getParentFragment().getParentFragment();
                        if (parent != null) {
                            parent.requestTransitSonglistScreen(targetMediaId);
                        }
                    }
                })
        );
